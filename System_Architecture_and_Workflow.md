...
flowchart TD
    ...
    C --> D["🧠 CNN Feature Extraction<br>ResNet-18 (pretrained on ImageNet)<br>Each frame → 512 numbers"]
    ...
    F --> G["📍 Highlight Selection<br>Keep events with probability > 50%<br>Remove duplicates (NMS)"]
...
flowchart TD
    ...
    subgraph CNN_BLOCK["🔍 CNN Feature Extractor"]
        ...
        NORM --> RESNET["ResNet-18 (pretrained, frozen)<br>conv1 → bn1 → relu → maxpool<br>layer1 → layer2 → layer3 → layer4<br>→ avgpool"]
        RESNET -->|"Remove FC layer"| FEAT["Feature Vectors<br>N × 512"]
    end
...
| Parameter | Value | Why |
|:---|:---|:---|
| **ResNet-18** (not 50/152) | 512-dim features | Lighter model, faster on Colab; SoccerNet already provides ResNet features |
...
| Component | What They Did | How We Adapt It |
|:---|:---|:---|
| ResNet feature extraction | `ResNet18 → remove FC → avgpool output` | Exact same approach for raw video processing |
